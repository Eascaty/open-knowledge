...
4. 空库首页可选择“用虚构示例试一遍”，先预览并编辑文字，再点击“投入知识库”；已有草稿不会被覆盖。也可选择“导入自己的资料”，或投放包内 `examples/java_g1.md`。整理完成后打开知识卡，搜索“间隔复习”（网页示例）或“Java”（文件示例）。示例作为普通私密资料保存，不会自动导入。
...
用本机知识管家打开知识卡，在“原始来源”点击“核对本机原文”。页面显示对应行号和经过摘要核对的文本，可再次点击收起。当前支持文本、Markdown、代码及 DOCX/HTML 提取文本；后两者的行号不代表原文件页码或排版。PDF 和旧版 DOC 暂需在本机原始资料中核对。原件缺失或摘要改变时先检查备份，不会自动覆盖原件。离线分享包和公开站不提供原件入口。

...
- Python：98/98 通过，包含真实本机回环 HTTP、启动状态回滚、人工归类与可信审核事务、同源接口、上传安全、长文拆卡、v1→v2 迁移、迁移后自动续跑、恢复演练和队列/候选站语义。
- Web：30/30 通过（静态/API 数据源 2 项，相关知识 2 项，网页投放 10 项，本机归类与撤销 4 项，可信审核 4 项，可信审核队列 4 项，Markdown 阅读器 4 项）。

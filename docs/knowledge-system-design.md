...
- 每日生成健康报告。
- 数据库创建一致性快照。
- `scripts/backup-bundle` 在本地生成包含数据库快照、原始资料、Vault、站点和配置的私密归档；清单逐文件记录 SHA-256。ZIP 先写入私密临时候选，关闭归档后自动验证逐文件摘要与 SQLite 快照，全部通过才原子发布最终 ZIP；失败清理候选并保留已有成功备份。核验不覆盖正式库，也不等于整包恢复到新目录。

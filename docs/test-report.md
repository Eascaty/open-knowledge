...
- 结果：当前变更可在沙箱运行的 94 项非回环 Python 测试与41个 Web 场景全部通过；完整 Python 套件共117项，其中知识管家回环 HTTP/进程测试因当前沙箱禁止端口绑定，需在本机环境复验；Java 38项测试在隔离 JDK 21 环境通过，J4 Linux 容器冒烟由远程 CI 复验
...
- 完整项目备份包覆盖配置、raw、Vault、站点和 SQLite 快照；逐文件哈希、符号链接和篡改包拒绝通过
- Java/G1 深层母子分类
- 未知资料进入“待归类”
- 非法跨级模型路径被拒绝
- 失败重试和最终隔离

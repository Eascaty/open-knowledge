...
- 人工归类安全底座支持预演、活动节点校验、乐观并发保护、原子更新搜索路径和审计事件；当前通过 `scripts/reclassify` 使用，网页选择器尚未接入。
...
- Python：92/92 通过，包含真实本机回环 HTTP、启动状态回滚、人工归类事务、上传安全、长文拆卡、v1→v2 迁移、迁移后自动续跑、恢复演练和队列/候选站语义。

...
空格分隔关键词分别匹配，所有词均命中才返回；单个中文词保留连续子串规则。不输入关键词时可按路径、标签或可信状态浏览，最多显示30条；提示明确这是展示上限。清除筛选保留查询词，关闭搜索保留本次页面内筛选但清空查询。此轮不扩展 Java 搜索接口，不保存或上传用户查询；真实使用查询质量评价仍待后续验收。
...
从搜索结果打开知识卡时，页面内存暂存当前查询、筛选、列表滚动位置及卡片 ID，知识卡提供返回入口。返回时重新执行相同条件并恢复结果焦点；打开专业节点不保留该返回快照。快照不写 localStorage、URL、SQLite 或日志，刷新即清除。

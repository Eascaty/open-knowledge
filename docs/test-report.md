...
- 本轮运行环境：macOS、Python 3.9.6；完整回环测试使用允许绑定本机临时端口的环境。
- 本轮结果：Python 118/118、Web 41/41 通过。初次受限环境的回环测试因端口绑定被拒绝而失败；随后完整复验通过，未改动真实知识库或运行中的真实知识管家。
- 提交前复验：Python 118/118（30.732秒）、Web 41/41 通过；规划文档相对链接与 `git diff --check` 通过。
- 备份定向验证：4/4 通过。虚构 Markdown 经完整流水线生成来源和知识卡后打包；归档成员损坏时拒绝候选，验证旧备份、数据库与原件字节不变且候选已清理。
- 本轮没有重新运行 Java、容器或完整浏览器验收；本轮新增行为仍待提交及远端 PR 检查。

## 既有验收记录

以下为此前阶段的记录，不表示在本轮全部重新执行。此前 Java 环境为 Temurin JDK 21.0.12、Maven 3.9.11，38项测试通过；J4 Linux 容器由远程 CI 验证。
...
- 完整项目备份包覆盖配置、raw、Vault、站点和 SQLite 快照；逐文件哈希、符号链接和篡改包拒绝通过；归档生成后自动验包，损坏候选不发布且旧备份保持不变
- Java/G1 深层母子分类
- 未知资料进入“待归类”
- 非法跨级模型路径被拒绝
- 失败重试和最终隔离
...
- `scripts/publish-plan` 覆盖项目名校验、私密/公开门禁和不调用网络上传的 CLI 边界
- 完整项目备份覆盖数据库一致性快照、配置/收件箱/raw/Vault/站点产物清单、日志/WAL/SHM 排除、路径安全、逐文件摘要和候选库恢复验签

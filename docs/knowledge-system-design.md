...
复核队列打开卡片时，在当前控制器内存中保留卡片 ID、状态筛选、已展开数量及实际滚动容器的位置。仅对应卡片显示返回入口；返回时重绘原队列、恢复焦点和滚动，不改写审核状态、数据库或浏览器存储。公开站及本机审核不可用时无队列控制器，也无返回入口。刷新以及审核保存触发的页面重载会清除上下文，尚不提供跨刷新或暂缓复核语义。
